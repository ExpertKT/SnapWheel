...
**截完不用找文件：图就挂在屏幕角上，拖一下就发出去了。**

![SnapWheel 演示](docs/demo.gif)

| | |
|---|---|
| 🎯 **转角即取** | 截图、缩略图、拖拽发送全在屏幕角落完成 —— 不切窗口、不翻文件夹 |
| ↔️ **拖出去就发** | 把角上的缩略图直接拖进微信 / QQ / 文档 / 文件夹，松手即送达 |
| 📜 **滚动长截图** | 在浮层里框一块区域，剩下的交给它自己滚、自己拼，到底自动停 |
| 🔍 **取字 + 翻译** | 圈住文字就认出来并复制；一键翻译，默认走免费接口、零配置 |
| ↩️ **删错了能找回来** | 撤销上一次删除，最近 8 次都记得住；退出即清空，不建垃圾目录 |

一个 exe、免安装、不写注册表（自启项可选）、不打包任何模型。下面是它是什么、为什么用它、以及从哪开始。
# SnapWheel 快照轮环
