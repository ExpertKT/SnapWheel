...
namespace SnapWheel
{
    ...
    {
#if NO_KEY
        public const string Version = "0.2.11";   // 变体：多 Wheel + 框选缩放/锁定（无万能键）
#else
        public const string Version = "0.4.1";   // 完整版：含万能键摇杆 + 旋转 + 缩放修正
#endif
    ...
    class WheelForm : Form
    {
        ...
        void BackdropClip(Graphics g, GraphicsPath path)
        {
            ...
            try
            {
                ...
                g.DrawImageUnscaled(_backdropBlur, _backdropOffset.X, _backdropOffset.Y);
                // 压一层黑：不管背后是亮桌面还是暗桌面，面板都能保持"深色玻璃"、字看得清
                int dk = (int)(58 * (StyleSolid() ? 1f : _settings.GlassPercent / 100f));
        ...
        {
            return _settings.ShowNameLabel ? _namePillRect : RectangleF.Empty;
        }
        ...
        void DrawWheel(Graphics g, int w, int h)
        {
            ...
            if (pb0 > 0.01f)
            {
                ...
                Rectangle cbr0 = cbr;
                using (GraphicsPath cbp2 = new GraphicsPath()) { cbp2.AddEllipse(cbr0); BackdropClip(g, cbp2); cbp2.Dispose(); }
                Gfx.NeuCircle(g, cbr0, Gfx.A(GlassBase(), GlassA((int)((_closeHover ? 236 : 206) * ab0 / 255f))),
            ...
            if (pb1 > 0.01f)
            {
                ...
                using (GraphicsPath gbp2 = new GraphicsPath()) { gbp2.AddEllipse(gbr); BackdropClip(g, gbp2); gbp2.Dispose(); }
                Gfx.NeuCircle(g, gbr, Gfx.A(GlassBase(), GlassA((int)((_gearHover ? 236 : 206) * ab1 / 255f))),
            ...
            if (hasKey && a > 60 && pn > 0.01f && _settings.ShowNameLabel)
            {
                ...
                using (Font fw = new Font("Microsoft YaHei UI", 10f, FontStyle.Bold))
                {
                    ...
                    {
                        BackdropClip(g, pg2);
                        Gfx.GlassPanel(g, pg2, pill2, Gfx.A(GlassBase(), GlassA((int)((_nameHover ? 242 : 214) * an / 255f))),
            ...
            if (pb2 > 0.01f)
            {
                ...
                using (GraphicsPath sbp2 = new GraphicsPath()) { sbp2.AddEllipse(sbr); BackdropClip(g, sbp2); sbp2.Dispose(); }
            ...
            if (_store.Items.Count > 0 && _settings.ShowCountLabel)
            {
                ...
                using (Font f = new Font("Microsoft YaHei UI", fs, FontStyle.Bold, GraphicsUnit.Pixel))
                {
                    ...
                    {
                        BackdropClip(g, pg);
                        Gfx.GlassPanel(g, pg, pill, Gfx.A(GlassBase(), GlassA((int)(214 * ac2 / 255f))),
        ...
        void DrawToast(Graphics g, int a)
        {
            ...
            using (Font f = new Font("Microsoft YaHei UI", 10f))
            {
                ...
                {
                    BackdropClip(g, pp);
                    Gfx.GlassPanel(g, pp, new RectangleF(x, y, w, h), Gfx.A(GlassBase(), GlassA((int)(ta * 0.94f))),
